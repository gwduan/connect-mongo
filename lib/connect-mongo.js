...
module.exports = function(connect) {
  ...
  function MongoStore(options, callback) {
    ...
    if (options.mongoose_connection){
      ...
      if (Array.isArray(options.db.servers)) {
        var servers = [];
        for (var i = 0; i < options.db.servers.length; i++) {
          servers.push(new mongo.Server(options.db.servers[i].host,
                                        options.db.servers[i].port,
                                        options.db.servers[i].serverOptions)); 
        }
        if (options.db.rsOptions) {
          var replSet = new mongo.ReplSet(servers, options.db.rsOptions);
          this.db = new mongo.Db(options.db.name, replSet, {w: 0});
        } else {
          var mongos = new mongo.Mongos(servers, options.db.msOptions);
          this.db = new mongo.Db(options.db.name, mongos, {w: 0});
        }
